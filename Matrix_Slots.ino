// Raspberry Pico using Gary Metheringham's Backpack for HUB75 LED
// Code taken from https://github.com/mrohner/slot-machine
// And modified to work with Adafruit_Protomatter Library
...
Adafruit_Protomatter matrix(
   ...
   1); // number of panels high (- if alternate panel upside down (zig-zag scanning))
...
#define LED_RED_VERYLOW    (1 <<  11)
#define LED_RED_LOW        (5 <<  11)
#define LED_RED_MEDIUM     (7 << 11)
#define LED_RED_HIGH       (15 << 11)

#define LED_GREEN_VERYLOW  (1 <<  5)
#define LED_GREEN_LOW      (5 << 5)
#define LED_GREEN_MEDIUM   (7 << 5)
#define LED_GREEN_HIGH     (15 << 5)

#define LED_BLUE_VERYLOW    1// 3
#define LED_BLUE_LOW        5// 7
#define LED_BLUE_MEDIUM     7//15
#define LED_BLUE_HIGH       15//31
...
void loop() {
  ...
  if (scatter < 3) {
    ...
    if (win > (bet[bet_index]*10)) {
      drawTXT(8,26,"BIG WIN",7, LED_RED_HIGH);//x,y,with,hight,text,size of text,color
      matrix.show();
      delay(1000);
      drawTXT(0,26,"         ", 9,LED_BLACK);
    }
...
void display_paylines(){
   for (int l = 0; l<20;l++) {
      if(win_per_payline[l]){
        ...
        drawTXT(0,26,"Win ", 4,LED_BLUE_HIGH);
...
void display_free_spins(){
  drawTXT(0,26,"F", 1,LED_BLUE_HIGH);
  drawNumber(free_spins,6,26,2,LED_PURPLE_MEDIUM); //number,x,y,number of characters
  drawTXT(17,26,"W",1,LED_BLUE_HIGH);
  drawNumber(free_spin_win,23,26,4,LED_PURPLE_MEDIUM);
}


void coin_please(){
  drawTXT(0,26,"COIN PLS.",9, LED_RED_HIGH);//x,y,with,hight,text,size of text,color
...
// void drawTXT(int x,int y, const char* text,int size, uint16_t colour)
// {
